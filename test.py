...
def load_model(model_path, scaler, device="cpu", num_envs=1, num_actions=12):
    ...
    def eval_actuator_network(joint_pos, joint_pos_last, joint_pos_last_last, 
        ...
        scaled_inputs = scaler.transform(torch.concat([joint_pos.unsqueeze(-1), 
                                                      ...
                                                      ],axis=-1).view(num_envs*num_actions,6).cpu())
        scaled_inputs = torch.tensor(scaled_inputs, dtype=torch.float32, device=device)
        torques = actuator_net(scaled_inputs.view(num_envs,num_actions, 6)).view(num_envs, num_actions)
...
if __name__ == '__main__':
    ...
    dataset, scaler, rawdata = load_data(os.path.dirname(model_path),device)


    # 2) load model, please specify your model path correctly, since jit script issue, load the model on cpu
    actuator_network = load_model(model_path, scaler, device="cpu", num_actions=num_actions)
    ...
    for idx in range(300): 
        ...
        with torch.no_grad():
            ...
            estimated_torques.append(actuator_network(joint_pos_err, joint_pos_err_last, joint_pos_err_last_last, joint_vel, joint_vel_last, joint_vel_last_last))
    ...
    plt.plot([s[0][7] for s in actual_torques], label="actual",color='r')
    plt.plot([s[0][7].cpu() for s in estimated_torques], label="prediction",color='g')
    plt.legend()
    plt.show()
