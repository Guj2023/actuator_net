...
def load_model(
    model_path, scaler, use_scale=False, device="cpu", num_envs=1, num_actions=12
):
    ...
    def eval_actuator_network(
        ...
        torques = actuator_net(inputs.view(num_envs, num_actions, 6)).view(
...
if __name__ == "__main__":
    ...
    actuator_network = load_model(
        model_path, scaler, use_scale=use_scale, device="cpu", num_actions=num_actions
    )
    ...
    for idx in range(100):
        ...
        with torch.no_grad():
            ...
            dof_pos = np.array(
                [rawdata["motorStatePos_" + str(i)][idx] for i in range(num_actions)]
            ).reshape(num_envs, num_actions)
            dof_vel = np.array(
                [rawdata["motorStateVel_" + str(i)][idx] for i in range(num_actions)]
            ).reshape(num_envs, num_actions)
            dof_pos_desired = np.array(
                [rawdata["motorAction_" + str(i)][idx] for i in range(num_actions)]
            ).reshape(num_envs, num_actions)
            actual_torques.append(
                np.array(
                    [
                        rawdata["motorStateCur_" + str(i)][idx]
            ...
            dof_pos = (
                ...
                .view(num_envs, num_actions)
                .view(num_envs, num_actions)
            )
            dof_vel = (
                torch.tensor(dof_vel, device=device)
                .view(num_envs, num_actions)
                .view(num_envs, num_actions)
            )
            dof_pos_desired = torch.tensor(dof_pos_desired, device=device).view(
                num_envs, num_actions
            )
