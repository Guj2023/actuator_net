...
class DataProcess:
    def __init__(
        ...
        if os.path.isdir(raw_filepath):
            self.pd_data = pd.read_csv(os.path.join(raw_filepath, "actuator_data.csv"))
    ...
    def process_data(self):
        ...
        for idx in self.motors:
            input_data_list.append(
                self.pd_data.loc[
                    :, [tmp + "_" + str(idx) for tmp in self.input_data_name]
                ]
            )
            output_data_list.append(
                self.pd_data.loc[
                    :, [tmp + "_" + str(idx) for tmp in self.output_data_name]
                ]
            )

        input_data = np.concatenate([value.values for value in input_data_list], axis=0)
        output_data = np.concatenate(
            [value.values for value in output_data_list], axis=0
        )
        ...
        # Combine various features into a single array
        # Features include:
        # - Position error (current position error)
        # - Last position error (previous timestep)
        # - Last-last position error (two timesteps ago)
        # - Velocity (current velocity)
        # - Last velocity (previous timestep)
        # - Last-last velocity (two timesteps ago)
        feature = np.concatenate(
            [
                pos_error,  # Current position error
                pos_last_error,  # Position error from the previous timestep
                pos_last_last_error,  # Position error from two timesteps ago
                vel,  # Current velocity
                vel_last,  # Velocity from the previous timestep
                vel_last_last,  # Velocity from two timesteps ago
            ],
            axis=-1,  # Concatenate along the last axis (columns)
        )
        ...
        with open(os.path.join(self.datafile_dir, "scaler.pkl"), "wb") as f:
            ...
            pickle.dump(
                scaler_dict, f
            )  # NOTE, input_data is feature rather than scaled_feature

        print(
            "Process ambot motor data, data file saved at: {:}".format(
                os.path.join(self.datafile_dir, "motor_data.pkl")
            )
        )
        print(
            "Scaler file saved at: {:}".format(
                os.path.join(self.datafile_dir, "scaler.pkl")
            )
        )
